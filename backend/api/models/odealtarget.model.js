...
var modelName = 'Odealtargets';

const odealtargetSchema = new mongoose.Schema({
...
odealtargetSchema.plugin(uniqueValidator);

odealtargetSchema.statics = {
  ...
  monthOrgTarget: function(startDate, endDate){
    ...
    return this.db.useDb(context.workspaceId).model(modelName).aggregate([
      ...
      "targetClosedDeals":"$targetData.closedDeals","targetNewDealDollars":"$targetData.newDealsDollars",
  ...
  monthlyTargetsTable: function(user){
    ...
    return this.db.useDb(context.workspaceId).model(modelName).aggregate([
      ...
      {$addFields:{targetNewDeals:"$target.newDeals", targetClosedDeals:"$target.closedDeals", 
       targetNewDealsDollars:"$target.newDealsDollars", targetClosedDealsDollars:"$target.closedDealsDollars"}},
      {$project:{"target":0}} ,{$sort:{"_id":1}}
      ])
  }
}

const Odealtarget = mongoose.model(modelName, odealtargetSchema);

module.exports = Odealtarget;