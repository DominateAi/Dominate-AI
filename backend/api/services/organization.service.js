...
organizationData.expirationDate = "2121-04-05T05:11:03+0000";
...
function createDefaultUser(organizationData, resolve, reject) {
    //basically create an object userData with all info of a user like email, pass, workspaceId etc.
    var userData = {
        'email': organizationData.defaultUserEmailId,
    ...
    var context = {};
//in the context object, store user's email from the userData object just made above
    context.email = userData.email;
//in the context object, store user's workspaceId
    context.workspaceId = userData.workspaceId;
//using the set context method to set the context, this can then be accessed by 'getcontext'
    currentContext.setCurrentContext(context);
//create user, lead, customer, quotation empty collections by default
...
//we are assigning the role id of the admin of the firm to this user (there are many role ids that exist)

        userData.role = adminRole._id;
        //default image
        let config = configResolve.getConfig();
        userData.profileImage = config.defaultUserFileUrlPath;
//default user creation, adds a new user to the database
