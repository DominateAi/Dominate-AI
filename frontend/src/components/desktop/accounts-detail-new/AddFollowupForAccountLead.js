...
function AddFollowupForAccountLead({
  leadActivityData,
  buttonClassName,
  buttonText,
}) {
  ...
  useEffect(() => {
    if (!isEmpty(singleAccountData)) {
      let leadOption = [];
      if (!isEmpty(singleAccountData.leadsData)) {
        singleAccountData.leadsData.forEach((ele) => {
  ...
  const onSelectLeadDropdownSelect = (e) => {
    setValues({
      ...values,
  ...
  const handleSaveFollowUp = (e) => {
    ...
    const { errors, isValid } = validateAddFollowUp(values);
    if (!isValid) {
      // console.log(errors);
        ...
        const newFollowUp = {
          ...
          type:
            values.accountLeadsFollowUp === "SMS"
              ...
              : "CALL",
          entityType: "LEAD",
          entityId: values.selectedLeadOption,
          followupAtDate: values.startDate,
          followupAtTime: values.startTime,
          assigned: values.selectedLeadOption,
          assignedPipelead: leadActivityData._id,
          notification: true,
          status: "NEW",
        };
            ...
            <div className="add-lead-form-field-block add-follow-up-main-container">
              <div className="follow-up-select mb-30">
                ...
                <Select
                  ...
                  isSearchable={false}
                  options={values.selectLeadDropdownOptions}
                  value={values.selectedLead}
                  onChange={(e) => onSelectLeadDropdownSelect(e)}
                  placeholder="Select"
                />
                {!isEmpty(errors) && (
