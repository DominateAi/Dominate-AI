...
function CalendarNewLeavesRequestsUsers() {
  ...
  useEffect(() => {
    if (!isEmpty(allPendingAndApprovedLeaves)) {
      ...
      let filterLeaves = allPendingAndApprovedLeaves.filter(
        (leave) => leave.user._id === userData.id
      );
      console.log(filterLeaves, allPendingAndApprovedLeaves);
      setAllLeaves(filterLeaves);
    } else {
      setAllLeaves([]);
    }
...
export default CalendarNewLeavesRequestsUsers;
