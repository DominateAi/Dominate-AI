...
import { createLeadsPipelineStage } from "./../../../store/actions/leadsPipelineAction";
...
function AddNewStage() {
  ...
  const handleSubmitFunctionMain = () => {
    ...
    // }
    // if (isValid) {
    var leadPipelineData = JSON.parse(localStorage.getItem("leadPipelineData"));

    const formData = {
      leadStageName: values.stackName,
      pipeline: leadPipelineData._id,
      additionalInfo: "asdasdscsdc",
    };
    dispatch(createLeadsPipelineStage(formData, callBackCreateStack));
    // }
  };
    ...
    return (
      ...
      >
        ...
        <div className="add-lead-modal-container container-fluid pr-0">
          <h1 className="font-30-bold mb-61">New Stage</h1>
            ...
            >
              ...
              {prevNextIndex === 0 && (
                <AddEmployeesFormFields
                  type="text"
                  htmlFor={"stackName"}
                  labelName={"What is the stage's name?"}
                  id={"stackName"}
                  name={"stackName"}
                  placeholder={"Eg. name"}
  ...
  return (
    <Fragment>
      <div className="heads add_new_stack">
        <button className="add_new_stack_btn" onClick={onOpenModal}>
