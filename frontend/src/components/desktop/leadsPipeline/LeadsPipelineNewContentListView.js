...
function LeadsPipelineNewContentListView({
  overviewFilterName,
  defaultOption,
}) {
  ...
  //REDUCERS
  const allLeads = useSelector(
    (state) => state.leadsPipeline.allLeadsOfPipeline
  );
  ...
  const onStatusClick = (e) => {
    ...
    if (!isEmpty(allPipelineStage)) {
      let filteredStage = allPipelineStage.filter(
        (stage) => stage.leadStageName === e.value
      );
      const leadFilter = {
        // "pageNo": 10,
        // "pageSize": 0,
        query: {
          stage: filteredStage[0]._id,
          pipeline: filteredStage[0].pipeline,
        },
      };
            ...
            {!isEmpty(filtereddata) ? (
              filtereddata.map((lead, index) => {
                ...
                return (
                  ...
                  index < values.currentPagination * totalRecordsInOnePage && (
                    <div
                      key={index}
                      className={
                        ...
                        <div className="leads-new-list-container__row1__colm5">
                          ...
                          (lead.assigned._id === userData.id ||
                            ...
                            <Dropdown
                              ...
                              value={renderCurrentStageName(lead)}
                            />
                          ) : (
                            renderCurrentStageName(lead)
                          )}
                        </div>
                      </div>
                      <div className="row mx-0 leads-new-list-container__row2">
                        ...
                        <div className="row mx-0 leads-new-list-container__row2-block-img-text">
                          <img
                            ...
                            className="note-img"
                          />
                          <span onClick={addToLogHandler(lead)}>add logs</span>
                        </div>
                        {/* kanban */}
...
export default LeadsPipelineNewContentListView;
