...
class OverviewDemoModalFirstTimeUser extends Component {
  state = {
  ...
  onCloseModal = () => {
    let newData = this.props.userData;
    newData.demo = false;
    console.log(newData);
    this.props.updateUserDemoWalkthroughFlag(this.props.userData.id, newData);
    this.setState({
      open: false,
      prevNext: 0
    });
...
export default connect(mapStateToProps, { updateUserDemoWalkthroughFlag })(
  OverviewDemoModalFirstTimeUser
);
