...
export const validateAddAccount = (data) => {
  let errors = {};
  ...
  let aboutAccount = data.aboutAccount.trim();
  ...
  //About Account
  if (!Validator.matches(aboutAccount, /^[a-zA-Z0-9\s]+$/)) {
    errors.aboutAccount =
      "Please enter a valid description using only a-z, A-Z 0-9";
  }

  if (Validator.isEmpty(aboutAccount)) {
    errors.aboutAccount = "Account description  is required";
  }
